    ...
    if('txhash' in record): 
        ...
        try: 
            if(doc != None):
                ...
                row = {"txhash": record['txhash'], "blocknumber": int(doc['blocknumber'],16), "blocktime": end_time,"waiting_time": 0.0,"actual_cost": 0.0, "gas_price":0.0, "waiting_mined_time": waiting_mined_time}        
    ...
    if('txhash' in record): 
        ...
        try: 
            if(doc != None):
                ...
                end_time = int(end_time,16)
                waiting_mined_time = end_time - start_time
                row = {"txhash": record['txhash'], "blocknumber": int(record['blocknumber'],16), "blocktime": end_time,"waiting_time": 0.0,"actual_cost": 0.0, "gas_price":0.0, "waiting_mined_time": waiting_mined_time}        
                result = col_summary.insert_one(row)
                print('inserted')
                return 0
            else:
