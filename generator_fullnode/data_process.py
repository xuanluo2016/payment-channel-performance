...
def get_waiting_mined_time():
    ...
    query = """
        ...
        SELECT  count(*) AS count,txhash, min(starttime) As starttime, gasprice FROM transactionsdb.startnodelay
        group by txhash
        """
    ...
    for row in cursor:
        x.append(row[0])
        y.append(int(row[1],16)/1000000000)
        z.append(row[2])
...
def get_waiting_mined_time_from_one_node(hostname):
    ...
    query = """
        select (t2.blocktime - t1.starttime),t1.gasprice, t2.blocktime, t1.txhash
        from transactionsdb.startnodelay as t1, transactionsdb.end as t2
        where t1.txhash = t2.txhash and t1.hostname =  """ + hostname + """ 
        order by t1.starttime
        """
    cursor.execute(query)
    for row in cursor:
        x.append(row[0])
        y.append(int(row[1],16))
        z.append(row[2])
